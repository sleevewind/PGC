...
class PGC(nn.Module):
    def __init__(self, dim, n_head, num_embedding=500):
        super().__init__()
        self.proj = nn.Linear(dim, dim)
        self.self_attn_cls = nn.MultiheadAttention(dim, n_head, batch_first=True)
        self.box = nn.Parameter(torch.linspace(-2, 2, num_embedding).unsqueeze(1), requires_grad=False)
        self.attn_box_proj = nn.Linear(dim, 4 * num_embedding)
        self.norm1 = nn.LayerNorm(dim)
        self.norm2 = nn.LayerNorm(dim)

    def forward(self, tgt, attn_mask):
        tgt_cls = self.norm1(tgt + self.proj(tgt))
        _tgt_cls, _ = self.self_attn_cls(tgt_cls, tgt_cls, value=tgt_cls, attn_mask=attn_mask)
        tgt_cls = self.norm2(tgt_cls + _tgt_cls)

        v = self.box.unsqueeze(0).tile([tgt.size(0), 1, 1])
        attn_box = rearrange(self.attn_box_proj(tgt), 'b m (k n) -> k b m n ', k=4)
        attn_box = F.softmax(attn_box, dim=-1)
        tgt_box = attn_box @ v.unsqueeze(0)
        tgt_box = rearrange(tgt_box, 'k b m d -> b m (k d)', k=4)
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(
        self, target, target_cls, reference_points, value, spatial_shapes, attn_mask=None, query_pos_embed=None
    ):

        target = target + self.proj(target_cls.detach())
        ...
        tgt_cls, tgt_box = self.pgc(target, attn_mask)

        return target, tgt_cls, tgt_box
...
class TransformerDecoder(nn.Module):
    ...
    def forward(
        ...
        for i, layer in enumerate(self.layers):
            ...
            output, output_cls, output_box = layer(
                output, output_cls, ref_points_input, value, spatial_shapes, attn_mask, query_pos_embed
            )

            if i == 0:
                # Initial bounding box predictions with inverse sigmoid refinement
                pre_bboxes = F.sigmoid(pre_bbox_head(output) + inverse_sigmoid(ref_points_detach) + output_box)
                pre_scores = score_head[0](output_cls)
            ...
            inter_ref_bbox = F.sigmoid(inverse_sigmoid(inter_ref_bbox) + output_box)

            if self.training or i == self.eval_idx:
                scores = score_head[i](output_cls)
